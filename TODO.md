...
- [ ] `optimize-instructions` / `precompute` / `local-cse` を優先移植する（precompute: `i32.const+i32.const+i32.add`、simplify-locals: `local.set+local.get -> local.tee`、drop-elision: `local.get/global.get/ref.func/ref.null + drop` は実装済み）
