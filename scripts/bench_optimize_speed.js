...
function run_one_case(root_dir, binary_path, benchmark_case, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    const output_path = path.join(
      os.tmpdir(),
      `wite-opt-speed-${process.pid}-${Date.now()}-${i}.wasm`,
    )
    const args = ["optimize", benchmark_case.input, output_path, ...benchmark_case.flags]
...
function main() {
  ...
  const cases = [
    {
      name: "core_optimize_o1_zlib",
      input: "bench/corpus/core/binaryen/zlib.wasm",
      flags: ["-O1"],
    },
    {
      name: "core_optimize_o2_zlib",
      input: "bench/corpus/core/binaryen/zlib.wasm",
      flags: ["-O2"],
    },
  ]
  if (options.include_pglite) {
    cases.push({
      name: "core_optimize_o1_pglite",
      input: "bench/corpus/core/pglite/pglite.wasm",
      flags: ["-O1"],
    })
  }
